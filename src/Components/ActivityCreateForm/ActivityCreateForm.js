...
// React controlled form to find and create activities
function ActivityCreateForm() {
  ...
  // handle function for ActivitySearch Component
  const handleSearch = async (data) => {
    const res = await TourApi.searchPlaces({
      lat: tourstop.lat,
      lng: tourstop.lng,
      origin_id: tourstop.googleplaces_id,
      mode: data.mode,
      duration: data.traveltime,
      query: data.keyword
    })

    // get lat and lng numbers using google fromPlaceId func and store them in state
    const geoPromises = []
  ...
  // create new Activity based on selected Data
  const createActivity = async () => {

    let poi = {}
    // post poi or get existing location from db
    const res = await TourApi.postPoi({
      name: selected.name,
  ...
  if (user.token && tourstop) {
    return (
    <div className="ActivityCreateForm">
